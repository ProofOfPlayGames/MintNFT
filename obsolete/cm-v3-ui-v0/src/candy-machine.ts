...
export const getNftPrice = (candyMachineState: CandyMachineState, groupLabel?: string) => {
  ...
  if (groupLabel) {
    ...
    cost = candyMachineState.rawCandyMachine.candyGuard?.guards?.tokenPayment?.amount.basisPoints;
...
export const mint = async (
  ...
  try {
    const result: any = await metaplex.candyMachines().mint({
      ...
      collectionUpdateAuthority: new PublicKey("ERd6QhQRfaYLtq3zhhtfSVxqJAQQGH8HCLykpCVLADGP"),
      ...group && { group },
    });
